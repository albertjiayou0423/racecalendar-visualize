...
export function ShareCard({ event, now }: ShareCardProps) {
  ...
  useEffect(() => {
    const updateScale = () => {
      ...
      const s = Math.min(1, (containerWidth - 16) / CARD_WIDTH)
      setScale(s)
    }
    updateScale()
  ...
  return (
    <section className="mt-4 space-y-3">
      ...
      {/* 卡片预览 - 移动端自动缩放 */}
      <div ref={wrapperRef} className="w-full overflow-x-auto rounded-xl border border-border bg-black/5 p-4">
        <div
          className="mx-auto"
          style={{
            width: CARD_WIDTH,
          ...
          >
            ...
            <div className="relative z-10 flex h-full flex-col justify-between">
              ...
              <div className="flex items-center justify-between border-t border-white/10 pt-4">
                ...
                <span className="font-mono text-xs text-white/20">racecalendar-visualize.vercel.app</span>
              </div>
            </div>
          </div>
        </div>
